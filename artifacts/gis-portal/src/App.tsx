...
const VERSION_HISTORY = [
  {
    ...
    changes: [
      '打開關係宇宙頁面時，右上角會列出「上次造訪之後」新出現的人物/事件/案件/物件、L5 新寫的敘事段落、剛被刷新的樞紐觀察評價——每項都可以點下去直接跳到那個節點的詳情，不用自己在整張圖裡找',
      '判斷「新」的基準是「上次造訪時間」（存在瀏覽器本機），不是固定時間窗——按面板上的「知道了」才會把這個時間往前推，沒按的話下次打開還是看到同一批，不會因為只是瞄了一眼就被標記已讀；第一次使用這個功能會先抓最近 7 天當預設',
      '純前端運算，資料都已經在既有的關係圖 API 回應裡，沒有新增後端欄位或 API',
    ],
  },
  {
    version: '2.8.0',
