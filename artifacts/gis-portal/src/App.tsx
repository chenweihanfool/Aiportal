...
const VERSION_HISTORY = [
  ...
  },
  {
    ...
    changes: [
      '原本的算法是數符合「今天日期開頭.md」的檔案數量——但一天實際上只有一個 YYYY-MM-DD.md 檔，這樣算永遠是 1，沒有意義。改成讀取當天檔案的「內容」：「👤 人類編輯區」有真的寫東西（不是預留位置文字）算一篇，「🤖 AI 處理區」底下每個時間戳記條目（### HH:MM 開頭）各算一篇，但排除早安報告／週月季年報／L1-L4知識庫維護報告這些定期自動報告',
      '直接拿正式的日記檔案測試才抓到兩個問題：(1) 用裸的「L1」「L2」「L3」「L4」當排除關鍵字會誤殺只是提到這些字眼、內容其實是正常工作筆記的條目（例如「需歸檔：L3 架構釐清...」），改成只比對「知識庫維護報告」這個完整詞組；(2) 篇幅較長的技術類條目內部會用 ### 再分好幾個沒有時間戳的子段落（例如「問題」「處理」「狀態」這種標題），一開始沒過濾掉，會把子段落也算成獨立一篇，分數灌得不合理的高——改成必須是「### HH:MM 開頭」的標題才算數',
    ],
  },
  {
    version: '1.31.1',
            ...
            <div style={{ marginTop: '0.9rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                {personNames.map(name => (
                  <span key={name} style={{ fontSize: '11.5px', color: 'rgba(255,255,255,0.75)', background: 'rgba(192,132,252,0.12)', border: '1px solid rgba(192,132,252,0.3)', borderRadius: '999px', padding: '3px 10px' }}>{name}</span>
                ))}
              </div>
            </div>
          )}
